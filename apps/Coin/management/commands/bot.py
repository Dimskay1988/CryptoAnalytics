...
def user_register(message):
    if message.text == 'Использовать данные телеграм':
        ...
        Profile.objects.get_or_create(id_user=message.chat.id, name=message.from_user.first_name,
        ...
        bot.send_message(message.chat.id, 'Регистрация обязательна')
...
def new_last_name(message, last_name):
    ...
    # obj, created = Profile.objects.get_or_create(id_user=message.chat.id, name=last_name, surname=first_name, defaults={'id_user': message.chat.id, 'name': last_name, 'surname': first_name})
    Profile.objects.get_or_create(id_user=message.chat.id, name=last_name, surname=first_name)
